...
import { curveLpHelperDeployment, curveLpHelperNotZapDeployment } from "./deployment";
// eslint-disable-next-line node/no-missing-import
...
describe("CurveLpHelper tests", () => {
  it("deployment", async () => {
    const { usdt, aggregator, lpHelper, priceFeeds } = await loadFixture(curveLpHelperDeployment);
  ...
  it("deposit", async () => {
    const params = await loadFixture(curveLpHelperDeployment);
    ...
    await sa.setSwapPath(
      swapPathes.USDT_TO_USDC.tokenFrom,
      swapPathes.USDT_TO_USDC.tokenTo,
      swapPathes.USDT_TO_USDC.path
    );
...
describe("CurveLpHelper without zaps tests", () => {
  it("deployment", async () => {
    const { usdt, aggregator, lpHelper, priceFeeds } = await loadFixture(curveLpHelperNotZapDeployment);
  ...
  it("deposit", async () => {
    const params = await loadFixture(curveLpHelperNotZapDeployment);
    const { lpHelper, vault, usdt, deployer, sa } = params;
    await transferImpersonatedTokens({
      deployer: params.deployer,
      uniswapV3: params.uniswapV3,
