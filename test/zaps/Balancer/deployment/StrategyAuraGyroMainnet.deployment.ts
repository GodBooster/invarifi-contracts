...
export const StrategyAuraGyroMainnetDeployment = async () => {
  ...
  const strategyAuraGyroMainnet = await new StrategyAuraGyroMainnet__factory(deployer).deploy();

  const vault = await new VaultV7__factory(deployer).deploy();
  await vault.initialize(
    strategyAuraGyroMainnet.address,
    "Moo Aura wstETH/sfrxETH/rETH V3",
    "mooAurawstETH/sfrxETH/rETHV3",
    21600
  );

  const params = getBalancerAuraWSTETH_ETHInitializeParams({
    vault: vault.address,
  ...
  await strategyAuraGyroMainnet.initialize(...params);
