...
export const beefOut = async (
  ...
  const tokensBefore = await tokenOut.balanceOf(deployer.address);

  expect(await tokenOut.balanceOf(zap.address)).eq(0);
  expect(await vault.balanceOf(deployer.address)).gt(0);

  console.log(await tokenOut.balanceOf(deployer.address));

  await zap.beefOut(vault.address, withdrawAmount, 0, encodedDataToBeefOut);

  console.log(await tokenOut.balanceOf(deployer.address));
  const tokensAfter = await tokenOut.balanceOf(deployer.address);

  expect(tokensAfter.sub(tokensBefore)).gt(0);
  expect(await vault.balanceOf(deployer.address)).eq(0);
  expect(await tokenOut.balanceOf(zap.address)).eq(0);
  expect(await vault.balanceOf(zap.address)).eq(0);
...
export const beefOutAndSwap = async (
  ...
  expect(await tokenOut.balanceOf(zap.address)).eq(0);
  expect(await vault.balanceOf(deployer.address)).gt(0);
  console.log("OUT", await vault.balanceOf(deployer.address));

  console.log({ stable: (await tokenOut.balanceOf(deployer.address)).toString() });
