...
import "../interfaces/IStrategyEthCall.sol";
import "@chainlink/contracts/src/v0.8/automation/interfaces/KeeperCompatibleInterface.sol";

import "../libraries/UpkeepLibrary.sol";

contract VaultGasOverheadAnalyzer is
    ...
    KeeperCompatibleInterface
{
    ...
    IRegistry private _vaultRegistry;
        ...
        )
    {
        ...
        if (_lastUpdateCycle > oneWeekAgo) {
            ...
            return (
                false,
                bytes("VaultGasOverheadAnalyzer: Ran less than 1 week ago.")
            );
        }
        ...
        uint256 currentIndex = UpkeepLibrary._getCircularIndex(
            _index,
            0,
            vaults.length
        );
        ...
        IVault vaultToAnalyze = IVault(vaults[currentIndex]);

        (bool didHarvest, uint256 gasOverhead) = _analyzeHarvest(
            vaultToAnalyze
        );

        performData_ = abi.encode(currentIndex, didHarvest, gasOverhead);

        return (true, performData_);
    }

    function _analyzeHarvest(
        IVault vault
    ) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        IStrategyEthCall strategy = IStrategyEthCall(
            address(IVault(vault).strategy())
        );
    ...
    function _tryHarvest(
        IStrategyEthCall strategy
    ) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
    ...
    function performUpkeep(bytes calldata performData_) external override {
        (uint256 currentIndex, bool didHarvest, uint256 gasOverhead) = abi
            .decode(performData_, (uint256, bool, uint256));

        _runUpkeep(currentIndex, didHarvest, gasOverhead);
    }
    ...
    function _runUpkeep(
        ...
        if (didHarvest_) {
            _vaultRegistry.setHarvestFunctionGasOverhead(
                vaultAddress,
                gasOverhead_
            );
        }
        ...
        // Update index
        _index = UpkeepLibrary._getCircularIndex(
            currentIndex_,
            1,
            vaults.length
        );
