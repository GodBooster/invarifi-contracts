...
contract Harvester is ManageableUpgradeable, IHarvester {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Contracts.
    IRegistry public _vaultRegistry;
    ...
    uint256 public _vaultHarvestFunctionGasOverhead; // Estimated average gas cost of calling harvest(). TODO: this needs to live in Registry, and needs to be a `per vault` number.
    ...
    function initialize(
        ...
        _vaultRegistry = IRegistry(vaultRegistry_);
        ...
        )
    {
        ...
        if (numberOfVaultsToHarvest == 0) return (false, bytes("AutoHarvester: No vaults to harvest"));
    ...
    function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        IVault vault = IVault(vaultAddress_);
        ...
        )
    {
        ...
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(_vaultHarvestFunctionGasOverhead); // TODO: Pull this number from Registry.
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        IStrategy strategy = IStrategy(IVault(vault_).strategy());
