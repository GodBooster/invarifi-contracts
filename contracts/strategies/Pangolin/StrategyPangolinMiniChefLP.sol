...
contract StrategyPangolinMiniChefLP is StratFeeManagerAccessableInitializable {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IPangolinMiniChef(chef).withdraw(
                poolId,
                _amount.sub(wantBal),
                address(this)
            );
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 strategistFeeAmount = nativeBal.mul(fees.strategist).div(
            DIVISOR
        );
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IPangolinMiniChef(chef).userInfo(
            poolId,
            address(this)
        );
        return _amount;
    ...
    {
        uint256 pngReward = IPangolinMiniChef(chef).pendingReward(
            poolId,
            address(this)
        );
        ...
        if (rewarder != nullAddress) {
            (, uint256[] memory amounts) = IPangolinRewarder(rewarder)
                .pendingTokens(poolId, address(this), pngReward);
