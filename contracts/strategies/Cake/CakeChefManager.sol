    ...
    IERC1271
{
    ...
    modifier onlyWhitelist(address _cakeChef, uint256 _pid) {
        require(
            whitelistedStrategy[_cakeChef][_pid] == msg.sender,
            "!whitelisted"
        );
        _;
    }
    ...
    function whitelistStrategy(address _strategy) external onlyManager {
        ...
        (uint256 stratBal, , ) = ICakeV2Chef(_cakeChef).userInfo(
            _pid,
            address(this)
        );
    ...
    function proposeStrategy(
        ...
        require(
            ICakeBoostStrategy(_oldStrategy).poolId() ==
                ICakeBoostStrategy(_newStrategy).poolId(),
            "!pid"
        );
    ...
    function upgradeStrategy(
        address _cakeChef,
        uint256 _pid
    ) external onlyWhitelist(_cakeChef, _pid) {
    ...
    function isValidSignature(
        ...
        if (
            userHasRole(accessControlMain.KEEPER_ROLE(), signer) ||
            userHasRole(accessControlMain.OWNER_ROLE(), signer)
        ) {
            return 0x1626ba7e;
        } else {
            return 0xffffffff;
