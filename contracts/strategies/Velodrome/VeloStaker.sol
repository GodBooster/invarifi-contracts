...
contract VeloStaker is ERC20, BeSolidStaker {
    ...
    constructor(
        ...
        address _rewardPool,
        address _router
    ) BeSolidStaker(_ac, _name, _symbol, _reserveRate, _solidVoter) {
    ...
    function harvest() external {
        for (uint i; i < activeVoteLps.length; ++i) {
            ...
            ISolidlyGauge(rewardsGauge.bribeGauge).getReward(
                tokenId,
                rewardsGauge.bribeTokens
            );
            ISolidlyGauge(rewardsGauge.feeGauge).getReward(
                tokenId,
                rewardsGauge.feeTokens
            );

            for (uint j; j < rewardsGauge.bribeTokens.length; ++j) {
                uint256 tokenBal = IERC20(rewardsGauge.bribeTokens[j])
                    .balanceOf(address(this));
                if (
                    tokenBal > 0 && rewardsGauge.bribeTokens[j] != address(want)
                ) {
                    router.swapExactTokensForTokens(
                        tokenBal,
                        0,
                        routes[rewardsGauge.bribeTokens[j]],
                        address(this),
                        block.timestamp
                    );
                }
            ...
            for (uint k; k < rewardsGauge.feeTokens.length; ++k) {
                uint256 tokenBal = IERC20(rewardsGauge.feeTokens[k]).balanceOf(
                    address(this)
                );
                if (
                    tokenBal > 0 && rewardsGauge.feeTokens[k] != address(want)
                ) {
                    router.swapExactTokensForTokens(
                        tokenBal,
                        0,
                        routes[rewardsGauge.feeTokens[k]],
                        address(this),
                        block.timestamp
                    );
                }
