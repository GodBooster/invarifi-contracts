...
contract StrategyCurveConvex is StratFeeManagerAccessableInitializable {
    ...
    function initialize(
        ...
        for(uint256 i = 0; i < 10; i++) {
            try ICurveSwap(want).coins(i) returns (address) {}
            catch {
                poolSize = i - 1;
            }
        }
    ...
    function getNativeToDepositPath() external view returns (bytes memory) {
        return nativeToDepositPath;
    }
