...
contract StrategyConvex is StratFeeManagerAccessableInitializable {
...
//    address public constant metaPool = 0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7;
    ...
    function getNativeToDepositRoute() external view returns (address[] memory) {
    ...
    function _giveAllowances() internal {
        ...
        _approve(native, unirouter, type(uint).max);
        _approve(depositToken, pool, type(uint).max);
        if (zap != address(0)) _approve(depositToken, zap, type(uint).max);
        _approve(crv, crvPool, type(uint).max);
        _approve(cvx, cvxPool, type(uint).max);
    }

    function _removeAllowances() internal {
        _approve(want, address(booster), 0);
        _approve(native, unirouter, 0);
        _approve(depositToken, pool, 0);
        if (zap != address(0)) _approve(depositToken, zap, 0);
        _approve(crv, crvPool, 0);
        _approve(cvx, cvxPool, 0);
    }
