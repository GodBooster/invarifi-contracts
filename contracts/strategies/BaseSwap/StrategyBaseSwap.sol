...
contract StrategyBaseSwap is StratFeeManagerAccessableInitializable {
    ...
    function initialize(
        ...
        nft = _nft;
        (, output, escrowToken, , , , , , , ) = IBaseSwapNFT(nft).getPoolInfo();

        native = _outputToNativeRoute[_outputToNativeRoute.length - 1];
        require(
            _outputToNativeRoute[0] == output,
            "outputToNativeRoute[0] != output"
        );
    ...
    function _vestRewards() internal {
        if (block.timestamp > lastVestCall + 1 days) {
            uint256 indexLength = IBaseSwapVesting(escrowToken)
                .getUserRedeemsLength(address(this));
            if (indexLength > 0) {
                for (uint i; i < indexLength; ) {
                    (, , uint256 endTime, , ) = IBaseSwapVesting(escrowToken)
                        .getUserRedeem(address(this), i);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 amount, , , , , , , , ) = IBaseSwapNFT(nft).getStakingPosition(
            tokenId
        );
        return amount;
    }
    ...
    function setVestingRewards(
        bool _vestingRewards,
