...
contract StrategyChronosLP is StratFeeManagerInitializable, GasFeeThrottler {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            _withdraw();
            tokenId = IMaGauge(rewardPool).depositAll();
    ...
    function _withdraw() internal {
        if (balanceOfPool() > 0) {
            IMaGauge(rewardPool).withdrawAndHarvest(tokenId);
            tokenId = 0;
        }
    }
    ...
    function _harvest(address callFeeRecipient, bool onDeposit) internal whenNotPaused {
        if (tokenId > 0) IMaGauge(rewardPool).getReward(tokenId);
    ...
    function balanceOfPool() public view returns (uint256) {
        ...
        return IMaGauge(rewardPool).balanceOfToken(tokenId);
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        if (tokenId == 0) return 0;
        return IMaGauge(rewardPool).earned(tokenId);
    }
