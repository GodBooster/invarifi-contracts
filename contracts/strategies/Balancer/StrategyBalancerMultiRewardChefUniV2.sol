...
contract StrategyBalancerMultiRewardChefUniV2 is StratFeeManagerInitializable {
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IBeethovenxChef(chef).deposit(poolId, wantBal, address(this));
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IBeethovenxChef(chef).withdrawAndHarvest(poolId, _amount - wantBal, address(this));
    ...
    function rewardsAvailable() public view returns (uint256) {
        return IBeethovenxChef(chef).pendingBeets(poolId, address(this));
    }
    ...
    function retireStrat() external {
        ...
        IBeethovenxChef(chef).emergencyWithdraw(poolId, address(this));
