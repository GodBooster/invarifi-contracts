...
contract StrategyBalancerMultiReward is StratFeeManagerAccessableInitializable {
    ...
    function initialize(
        ...
        bool _balSwapOn,
        BalancerStructs.BatchSwapStruct[] calldata _nativeToInputRoute,
        BalancerStructs.BatchSwapStruct[] calldata _outputToNativeRoute,
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        if (balSwapOn) {
            IMinter minter = IMinter(
                IRewardsGauge(rewardsGauge).bal_pseudo_minter()
            );
    ...
    function swapRewardsToNative() internal {
        ...
        for (uint i; i < rewardTokens.length; ++i) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (
                    rewards[rewardTokens[i]].routerType ==
                    BalancerStructs.RouterType.BALANCER
                ) {
                    ...
                    rewards[rewardTokens[i]].routerType ==
                    BalancerStructs.RouterType.UNISWAP_V3
                ) {
                    UniV3Actions.swapV3(
                    ...
                    rewards[rewardTokens[i]].routerType ==
                    BalancerStructs.RouterType.UNISWAP_V2
                ) {
                    IUniswapRouterETH(rewards[rewardTokens[i]].router)
    ...
    function addRewardToken(
        address _token,
        address _router,
        BalancerStructs.RouterType _routerType,
