...
contract StrategyAuraGyroMainnet is StratFeeManagerAccessableInitializable {
    ...
    BalancerStructs.BatchSwapStruct[] public nativeToLp0Route;
    BalancerStructs.BatchSwapStruct[] public lp0ToLp1Route;
    ...
    function initialize(
        address _want,
        BalancerStructs.BatchSwapStruct[] memory _nativeToLp0Route,
        BalancerStructs.BatchSwapStruct[] memory _lp0ToLp1Route,
    ...
    function addLiquidity() internal {
        ...
        (address[] memory lpTokens, , ) = IBalancerVault(unirouter)
            .getPoolTokens(poolId);

        if (lpTokens[0] != native) {
            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerActionsLib
                .buildSwapStructArray(nativeToLp0Route, nativeBal);
            BalancerActionsLib.balancerSwap(
                unirouter,
                swapKind,
                _swaps,
        ...
        if (nativeBal > 0) {
            ...
            (uint256 lp0Amt, uint256 lp1Amt) = calcSwapAmount(lp0Bal);

            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerActionsLib
                .buildSwapStructArray(lp0ToLp1Route, lp1Amt);
            BalancerActionsLib.balancerSwap(
                unirouter,
                swapKind,
                _swaps,
    ...
    function calcSwapAmount(
        uint256 _bal
    ) public view returns (uint256 lp0Amt, uint256 lp1Amt) {
        lp0Amt = _bal / 2;
        lp1Amt = _bal - lp0Amt;

        (uint256 rate0, uint256 rate1) = IBalancerPool(want).getTokenRates();

        (, uint256[] memory balances, ) = IBalancerVault(unirouter)
            .getPoolTokens(IBalancerPool(want).getPoolId());
    ...
    function getNativeToLp0Assets() external view returns (address[] memory) {
        return nativeToLp0Assets;
    }

    function getLp0ToLp1Assets() external view returns (address[] memory) {
        return lp0Tolp1Assets;
    }
