...
contract FeeBatchV3 is Initializable, OwnableUpgradeable {
    ...
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            IUniswapRouter(unirouter).swapExactTokensForTokens(cuberaAmt, 0, wNativeToCuberaRoute, treasury, block.timestamp);
        } else {
        ...
        emit Harvest(wNativeBal, stableAmt, cuberaAmt, rewardPoolAmount, block.timestamp);
    }
    ...
    function setRoute(address[] calldata _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            require(_route[_route.length - 1] == address(cubera), "!cubera");
