...
contract FeeBatchV3SolidlyRouter is Initializable, OwnableUpgradeable {
    ...
    ISolidlyRouter.Routes[] public wNativeToCuberaRoute;
    ...
    function initialize(
        ...
        address _rewardPool, 
        address _unirouter,
        ISolidlyRouter.Routes[] memory _cuberaRoute,
        ...
        for (uint i; i < _cuberaRoute.length; ++i) {
                wNativeToCuberaRoute.push(_cuberaRoute[i]);
            }
    ...
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            ISolidlyRouter(unirouter).swapExactTokensForTokens(treasuryHalf, 0, wNativeToCuberaRoute, treasury, block.timestamp);
        } else {
    ...
    function setRoute(ISolidlyRouter.Routes[] memory _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            delete wNativeToCuberaRoute;
            require(_route[_route.length - 1].to == address(cubera), "!cubera");
             for (uint i; i < _route.length; ++i) {
                wNativeToCuberaRoute.push(_route[i]);
            }
        }
    }
