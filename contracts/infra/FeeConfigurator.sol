...
contract FeeConfigurator is AccessControlAbstract {
    struct FeeCategory {
        uint256 total; // total fee charged on each harvest
        uint256 batcher; // split of total fee going to fee batcher
    ...
    function initialize(address _ac, uint256 _totalLimit) public initializer {
    ...
    function getFeeCategory(
        ...
        if (_adjust) {
            ...
            fees.batcher = (fees.batcher * _totalFee) / DIVISOR;
    ...
    function setFeeCategory(
        ...
        uint256 batcher = DIVISOR - _call - _strategist;
