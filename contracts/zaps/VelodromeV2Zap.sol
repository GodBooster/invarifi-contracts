...
contract VelodromeV2Zap {
    ...
    constructor(address _router, address _WETH) {
        ...
        require(WETH == router.weth(), 'WETH address not matching Router.weth()');
    }
    ...
    function beefInETH (address vault, uint256 tokenAmountOutMin) external payable {
        require(msg.value >= minimumAmount, 'Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();

        _swapAndStake(vault, tokenAmountOutMin, WETH);
    }

    function beefIn (address vault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
    ...
    function beefOut (address vaultAddress, uint256 withdrawAmount) external {
        (IVault vault, IPool pair) = _getVaultPair(vaultAddress);
    ...
    function beefOutAndSwap(address vaultAddress, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        (IVault vault, IPool pair) = _getVaultPair(vaultAddress);
    ...
    function _getVaultPair (address vaultAddress) private pure returns (IVault vault, IPool pair) {
        vault = IVault(vaultAddress);
        pair = IPool(vault.want());
    }

    function _swapAndStake(address vaultAddress, uint256 tokenAmountOutMin, address tokenIn) private {
        (IVault vault, IPool pair) = _getVaultPair(vaultAddress);
        require(pair.factory() == router.defaultFactory(), 'Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
    ...
    function estimateSwap(address vaultAddress, address tokenIn, uint256 fullInvestmentIn) public view returns (uint256 swapAmountIn, uint256 swapAmountOut, address swapTokenOut) {
        (, IPool pair) = _getVaultPair(vaultAddress);
        require(pair.factory() == router.defaultFactory(), 'Incompatible liquidity pair'); // router.addLiquidity adds to pair from router.defaultFactory()
