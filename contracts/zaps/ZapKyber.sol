...
contract ZapKyber {
    ...
    function beefInETH (address vault, bytes memory token0, bytes memory token1) external payable {
        require(msg.value >= minimumAmount, 'Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();
        _swapAndStake(vault, WETH, WETH, token0, token1);
    }

    function beefIn (address vault, address inputToken, uint256 tokenInAmount, bytes memory token0, bytes memory token1) public {
        require(tokenInAmount >= minimumAmount, 'Insignificant input amount');
        require(IERC20(inputToken).allowance(msg.sender, address(this)) >= tokenInAmount, 'Input token is not approved');

        IERC20(inputToken).safeTransferFrom(msg.sender, address(this), tokenInAmount);

        _swapAndStake(vault, inputToken, inputToken, token0, token1);
    }

    function beefOut (address vault, uint256 withdrawAmount) external {
        address[] memory tokens = _beefOut(vault, withdrawAmount);
         _returnAssets(tokens);
    }

    function _beefOut (address vaultAddress, uint256 withdrawAmount) internal returns (address[] memory tokens) {
        (IVault vault, IUniswapV2Pair pair) = _getVaultPair(vaultAddress);

        IERC20(vaultAddress).safeTransferFrom(msg.sender, address(this), withdrawAmount);
    ...
    function beefOutAndSwap(address vaultAddress, uint256 withdrawAmount, address desiredToken, bytes memory dataToken0, bytes memory dataToken1) external {
        (IVault vault, IUniswapV2Pair pair) = _getVaultPair(vaultAddress);
    ...
    function quoteStableAddLiquidityRatio(IVault vault) external view returns (uint256 ratio0to1) {
            (,IUniswapV2Pair pairAddress) = _getVaultPair(address(vault));
            ...
            ISolidlyRouter router = ISolidlyRouter(IZapStrategy(vault.strategy()).unirouter());
    ...
    function quoteRemoveLiquidity(IVault vault, uint256 mooTokenAmt) external view returns (uint256 amt0, uint256 amt1, address token0, address token1) {
        uint256 withdrawFee = IZapStrategy(vault.strategy()).withdrawalFee();
        uint256 liquidity = mooTokenAmt * vault.balance() / vault.totalSupply();
        uint256 fee = withdrawFee > 0 ? liquidity * withdrawFee / 10000 : 0;
        liquidity = liquidity - fee;
        
        (, IUniswapV2Pair pair) = _getVaultPair(address(vault));
    ...
    function _getVaultPair (address vaultAddress) private pure returns (IVault vault, IUniswapV2Pair pair) {
    ...
    function _swapAndStake(address vaultAddress, address inputToken0, address inputToken1, bytes memory token0, bytes memory token1) private {
        (IVault vault, IUniswapV2Pair pair) = _getVaultPair(vaultAddress);
