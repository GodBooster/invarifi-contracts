...
import "../../../contracts/vaults/VaultV7.sol";
import "../../../contracts/interfaces/common/IERC20Extended.sol";
import "../../../contracts/strategies/Curve/StrategyConic.sol";
import "../../../contracts/strategies/Curve/ConicZap.sol";
import "../../../contracts/strategies/Common/StratFeeManager.sol";
import "../../../contracts/utils/UniswapV3Utils.sol";
...
//    StrategyConic.CurveRoute nativeToUnderlyingRoute = StrategyConic.CurveRoute(nativeToUnderlying, nativeToUnderlyingParams, 0);
    ...
    function setUp() public {
        ...
        if (vaultAddress != address(0)) {
            ...
            StratFeeManagerInitializable.CommonAddresses memory commons = StratFeeManagerInitializable.CommonAddresses({
                ...
                feeRecipient: PROD_STRAT.feeRecipient(),
                feeConfig: PROD_STRAT.feeConfig()
            });
    ...
    function test_zapIn() external {
        ...
        IVault _vault = IVault(address(vault));

        vm.expectRevert('Input token not present in pool');
        zap.estimateSwap(_vault, cvx, 1000);

        address tokenIn = strategy.underlying();
        uint amount = 10000000000000000;
        (uint swapAmountIn, uint swapAmountOut, address swapTokenOut) = zap.estimateSwap(_vault, tokenIn, amount);
        ...
        if (tokenIn == native) {
            ...
            zap.beefInETH{value: amount}(_vault, amountMin);
        } else {
            IERC20(tokenIn).approve(address(zap), type(uint).max);
            zap.beefIn(_vault, amountMin, tokenIn, amount);
        }
        ...
        uint mooBal = _vault.balanceOf(address(this));
        uint tokenBal = mooBal * _vault.balance() / _vault.totalSupply();
    ...
    function test_zapOut() external {
        ...
        vm.expectRevert('desired token not present in pool');
        zap.estimateSwapOut(_vault, cvx, 1000);

        uint lpAmount = 10000;
        deal(want, address(this), lpAmount);
        IERC20(want).approve(address(vault), lpAmount);
        vault.deposit(lpAmount);
        uint withdrawAmount = _vault.balanceOf(address(this));

        address tokenOut = strategy.underlying();
        (uint swapAmountIn, uint swapAmountOut, address swapTokenIn) = zap.estimateSwapOut(_vault, tokenOut, withdrawAmount);
        ...
        _vault.approve(address(zap), type(uint).max);
        zap.beefOutAndSwap(_vault, withdrawAmount, tokenOut, amountMin);
